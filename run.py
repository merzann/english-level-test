...
def record_results(
    name, email, vocab_score, grammar_score, comprehension_score, cefr_level
):
    """
...
def save_results(
    ...
    """
    Saves results to results.json to be extracted upon sending results to user
    """
    results = {
        "name": name,
        "email": email,
...
while True:
    try:
        ...
        if not re.match(r"[^@]+@[^@]+\.[^@]+", email):
            raise ValueError(
                "Invalid email format. The format should be name@email.com"
            )
